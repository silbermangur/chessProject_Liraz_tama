...
// Constructor: Initializes the chessboard with pieces in their starting positions.
Board::Board()
{
...
// Deletes all dynamically allocated pieces on the board.
Board::~Board()
{
    int r = 0;
    int c = 0;
...
// Role: Prints the current state of the board
// Input: None
// Output: Displays the board to the console with piece symbols and empty spaces marked with '#'.
void Board::print()
{
    int r = 0;
    int c = 0;
...
// Role: Converts the board state into a string representation
// Input: None
// Output: Returns a string representing the board state, including player turn and filler character 'f'.
...
// Role: Moves a piece from one position to another
// Input: Source (fromR, fromC) and target (toR, toC) coordinates as characters.
// Output: Updates the board state or sets an error code if the move is invalid.
...
// Role: Checks if the current player is in check
// Input: None
// Output: Returns true if the current player's king is threatened, false otherwise.
